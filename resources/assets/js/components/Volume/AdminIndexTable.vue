<template>
    <div class="ui grid">
                ...
                <tr v-for="item in items">
                    ...
                    <td>
                        <template v-if="item.instance">
                            <router-link :to="{name: 'admin.computeInstances.dashboard', params: {id: item.instance.id}}" style="display: inline-block;">
                    ...
                    <user-column :entry="item"></user-column>
                    <td>
                        <i :class="item.node.zone.region.icon_class"></i> {{ item.node.zone.region.name }} - <router-link :to="{name: 'zones.dashboard', params: {id: item.node.zone.id}}">{{ item.node.zone.name }}</router-link> - <router-link :to="{name: 'computeNodes.show', params: {id: item.node.id}}">{{ item.node.name }}</router-link>
                    </td>
                    <td>
                    ...
                    <td>
                        <semantic-ui-dropdown-menu text="操作" v-bind:class="{loading: item.processing_operation_requests.length, disabled: item.processing_operation_requests.length}">
                            ...
                            <div v-else-if="!isAdmin && !item.instance" class="item" v-on:click="attach(item)">
        ...
        <form-modal ref="resizeModal" class="small" :custom-header="(volume2Change ? volume2Change.unique_id : '') + ' - 扩容'" :no-stay-select="true" submit-button-text="提交" v-on:submit="() => { $refs.resizeForm.resize() }" :is-loading="modalLoading">
            <resize-form :is-admin="isAdmin" :volume-price-per-gi-b-per-hour="volume2Change ? volume2Change.node.zone.storage_price_per_hour_per_gib : 0" ref="resizeForm" :volume="volume2Change" v-on:operationRequestCreated="() => {
        ...
        <form-modal ref="advanceSettingModal" class="small" :custom-header="(volume2Change ? volume2Change.unique_id : '') + ' - 高级设置'" :no-scroll="true" :no-stay-select="true" submit-button-text="提交" v-on:submit="() => {$refs.advanceSettingForm.changeBus();}" :is-loading="modalLoading">
            <advance-setting-form :is-admin="isAdmin" ref="advanceSettingForm" :volume="volume2Change" v-on:beforeSubmit="modalLoading = true" v-on:complete="modalLoading = false" v-on:operationRequestCreated="(operationRequest) => {$emit('operationRequestCreated', operationRequest)}" v-on:success="() => {filter(); $refs.advanceSettingModal.hide()}" v-on:saved="(bus) => {volume2Change.bus = bus;}"></advance-setting-form>
        </form-modal>
    </div>
</template>
...
<script>
    ...
    import UserColumn from "../AdminArea/ComputeInstance/Column/UserColumn";
    ...
    export default {
        name: "Volumes",
        props: ["isAdmin"],
        components: {UserColumn, AttachVolumeForm, NewVolumeForm, AdvanceSettingForm, ResizeForm, VolumeBus, Status},
