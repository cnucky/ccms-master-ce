    ...
    <form class="ui form" v-on:submit.prevent="$emit('submit')">
        <div class="two fields">
            <div class="ui field">
                ...
                <input type="text" placeholder="如：Red Hat Enterprise Linux" v-model="temporaryItem.name">
            </div>

            <div class="ui field">
                <label>{{ $t('common.iconClass') }}</label>
                <input type="text"  placeholder="如：fl-redhat" v-model="temporaryItem.icon_class">
            </div>
        </div>
