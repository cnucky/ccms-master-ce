<template>
    <form-modal ref="modal" class="medium" v-on:submit="$refs.form.$refs.submitButton.click()" :model-name="$t('common.publicISOImage')" :item-name="itemName" :is-loading="isLoading" :is-editing="isEditing">
        <public-floppy-form ref="form" v-on:submit="submit" :options="options" :is-editing="isEditing"></public-floppy-form>
    </form-modal>
</template>

<script>
    import PublicFloppyForm from "./PublicFloppyForm";
    export default {
        name: "PublicFloppyFormModal",
        components: {PublicFloppyForm},
        props: ["options"],
        data: function () {
            return {
                isEditing: false,
        ...
        methods: {
            ...
            submit: function () {
                ...
                if (this.isEditing) {
                    finallyCallback = axios.put(route("publicFloppies.update", [this.$refs.form.item.id]), this.$refs.form.temporaryItem, axiosConfig).then((response) => {
                    ...
                    finallyCallback = axios.post(route("publicFloppies.store"), this.$refs.form.temporaryItem, axiosConfig).then((response) => {
                        if (response.data.result) {
                            this.$refs.modal.positiveMessage("保存成功");
