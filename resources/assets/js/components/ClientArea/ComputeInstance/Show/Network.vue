            ...
            <table class="ui unstackable fixed table">
                ...
                <template v-for="(publicNetworkInterface, publicNetworkInterfaceIndex) in publicNetworkInterfaces">
                    ...
                    <template v-for="(ipv4Address, ipv4AddressIndex) in publicNetworkInterface.ipv4_addresses">
                        <i-p-address-table-row :interface-index="publicNetworkInterfaceIndex" :ip-address-index="ipv4AddressIndex" :ip-address="ipv4Address" v-on:unbind="unbindIPv4" v-on:release="releaseIPv4" :is-v4="true"></i-p-address-table-row>
                    </template>
                    <template v-for="(ipv6Address, ipv6AddressIndex) in publicNetworkInterface.ipv6_addresses">
                        <i-p-address-table-row :interface-index="publicNetworkInterfaceIndex" :ip-address-index="ipv6AddressIndex" :ip-address="ipv6Address" v-on:unbind="unbindIPv6" v-on:release="releaseIPv6"></i-p-address-table-row>
                    </template>
                </template>
                </tbody>
            </table>
            ...
            <table class="ui unstackable fixed table">
                ...
                <template v-for="(networkInterface, interfaceIndex) in privateNetworkInterfaces">
                    ...
                    <template v-for="(ipv4Address, ipv4AddressIndex) in networkInterface.ipv4_addresses">
                        <i-p-address-table-row :interface-index="interfaceIndex" :ip-address-index="ipv4AddressIndex" :ip-address="ipv4Address" v-on:unbind="unbindIPv4" v-on:release="releaseIPv4" :is-v4="true"></i-p-address-table-row>
                    </template>
                    <template v-for="(ipv6Address, ipv6AddressIndex) in networkInterface.ipv6_addresses">
                        <i-p-address-table-row :interface-index="interfaceIndex" :ip-address-index="ipv6AddressIndex" :ip-address="ipv6Address" v-on:unbind="unbindIPv6" v-on:release="releaseIPv6"></i-p-address-table-row>
                    </template>
                </template>
                </tbody>
            </table>
...
<script>
    import NetworkInterfaceModel from "./NetworkInterfaceModel";
    ...
    export default {
        ...
        methods: {
            ...
            unbindIPv4: function (poolId, position, n, i) {
                this.unbind("ipv4Assignments", poolId, position, n, i)
            },
            unbindIPv6: function (poolId, position, n, i) {
                this.unbind("ipv6Assignments", poolId, position, n, i)
            },
            unbind: function (ipType, poolId, position, n, i) {
                this.ipOperation(ipType, "unbind", poolId, position, n, i);
            },
            releaseIPv4: function (poolId, position, n, i) {
                this.release("ipv4Assignments", poolId, position, n, i)
            },
            releaseIPv6: function (poolId, position, n, i) {
                this.release("ipv6Assignments", poolId, position, n, i)
            },
            release: function (ipType, poolId, position, n, i) {
                this.ipOperation(ipType, "release", poolId, position, n, i);
            },
            ipOperation: function (ipType, operation, poolId, position, n, i) {
                this.confirmModal()
                    .withHeader("提示")
                    .withContent("确定" + (operation === "unbind" ? "解绑" : "释放") + "指定IP？")
                    .withOnApprove(() => {
                        this.isLoading = true;
                        axios.post(route(this.operationRoutePrefix + ipType + "." + operation), {poolId: poolId, position: position}, {vueInstance: this})
                            .then((response) => {
                                var data = response.data;
                                if (data.result) {
