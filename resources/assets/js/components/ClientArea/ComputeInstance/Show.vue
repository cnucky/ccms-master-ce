    ...
    align-items: center;">
            ...
            <div class="sixteen wide column">
                <div class="ui secondary pointing menu">
                    <router-link class="item teal" :to="{name: 'computeInstances.dashboard', params: {id: instance.id}}" exact-active-class="active">
                        概览
                    </router-link>
                    <router-link class="item teal" :to="{name: 'computeInstances.statistics', params: {id: instance.id}}" exact-active-class="active">
                        资源统计图
                    </router-link>
                    <router-link class="item teal" :to="{name: 'computeInstances.volumes', params: {id: instance.id}}" active-class="active">
                        卷
                    </router-link>
                    <div class="item teal disabled">
                        快照
                    </div>
                    <router-link class="item teal" :to="{name: 'computeInstances.virtualMedias', params: {id: instance.id}}" active-class="active">
                        虚拟介质
                    </router-link>
                    <router-link class="item teal" :to="{name: 'computeInstances.network', params: {id: instance.id}}" active-class="active">
                        网络
                    </router-link>
                    <router-link class="item teal" :to="{name: 'computeInstances.settings', params: {id: instance.id}}" active-class="active">
                        设置
                    </router-link>
                    <router-link class="item teal" :to="{name: 'computeInstances.histories', params: {id: instance.id}}" active-class="active">
                        操作历史
                    </router-link>
                    <div class="right menu">
                        ...
                        <a class="ui item teal" target="_blank" :href="route('computeInstances.console', [instance.id])">
            ...
            <div class="sixteen wide column">
                <slide-fade-transition>
                    <router-view ref="child" operation-route-prefix="" :instance="instance" :show-component="getVueInstance" v-on:operationRequestCreated="operationRequestCreated" v-on:instanceUpdated="(callback) => {callback(instance);}"></router-view>
                </slide-fade-transition>
            </div>
        </div>
    </div>
    ...
    export default {
        ...
        methods: {
            updateInstanceInformation: function (onComplete = undefined) {
                axios.get(route('computeInstances.show', [this.$router.currentRoute.params.id]))
                    .then((response) => {
            ...
            powerAction: function (powerAction) {
                // this.isLoading = true;
                axios.post(route('computeInstances.power.' + powerAction, [this.instance.id]), null, { vueInstance: this })
                    .then((response) => {
                        var data = response.data;
                        if (data.result) {
